...
mapping is shown in Table~\ref{tab:schema}. Three HyperKvasir classes
(\texttt{bbps-0-1}, \texttt{impacted-stool}, \texttt{out-of-patient}) were
excluded as they represent image quality or non-pathological states without
clinical risk stratification relevance.
...
Training augmentation comprised: random horizontal and vertical flips
($p=0.5$ each), random rotation ($\pm30^{\circ}$), colour jitter
(brightness\,$=$\,0.2, contrast\,$=$\,0.2, saturation\,$=$\,0.1,
hue\,$=$\,0.05), and random affine transformation ($\pm10^{\circ}$ rotation,
$\pm10\%$ translation). Validation and test images received only CLAHE
preprocessing and centre-crop normalisation. All images were resized to
$224\times224$ pixels and normalised using ImageNet population statistics
...
1 & Inflammatory  & 2.0 & Delayed medical treatment; mucosal progression\\
...
\begin{equation}
  \text{CD-CTEI} = \min_{d,c}\,
    \frac{F_{1,c}^{(d)}}{F_{1,c}^{\text{overall}}}
  \label{eq:cdctei}
\end{equation}

A CD-CTEI\,$\geq$\,0.95 indicates that no subgroup experiences more than
5\,\% relative performance degradation relative to the population average.
