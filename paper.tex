...
subgroups. The best model (EfficientNet-B0) reaches macro F1\,$=$\,0.83 on HyperKvasir
and 0.76 zero-shot on Kvasir-v2, with 43.9\,\% workload reduction and zero
missed High-Risk cases across all architectures.
\end{abstract}
...
any adaptation or fine-tuning. EfficientNet-B0 achieves the best
cross-dataset macro F1 (0.7645) with the smallest F1 drop (0.0674),
suggesting stronger generalisation than DenseNet-121 despite similar
in-domain performance. Notably, EfficientNet-B0 achieves 99.4\,\%
...
EfficientNet-B0 & \textbf{0.8319} & \textbf{0.7645} & \textbf{0.0674} & \textbf{0.9935}\\
DeiT-Tiny       & 0.8134 & 0.7603 & 0.0531 & 0.9695\\
\bottomrule
\end{tabular}
\end{table*}
...
The key methodological advances over prior work are threefold. First, the
...
Table~\ref{tab:ablation} reports macro F1-score and High-Risk recall
...
recall at 10-epoch training depth. The full 25-epoch training used in the
main experiment amplifies these differences---the AEL-trained DenseNet-121
achieves macro F1\,=\,0.8270 versus the 10-epoch AEL run at 0.8432,
confirming that longer training benefits all loss conditions but that AEL
maintains the macro F1 advantage throughout.
...
assessment. Constructing demographically annotated multi-site GI endoscopy
datasets remains an open and pressing need, particularly given evidence of
differential cancer incidence and endoscopic miss rates across age, sex,
and ethnicity subgroups~\citep{Kaminski2017}.
...
clinically grounded misclassification cost asymmetry. The best model
(EfficientNet-B0) achieves macro F1\,=\,0.8319 on HyperKvasir and
0.7645 zero-shot on Kvasir-v2, with High-Risk recall of 95.1\,\% and
43.9\,\% endoscopist workload reduction. All three architectures achieve
zero missed High-Risk lesions under the MC Dropout referral protocol,
confirming the clinical safety of the proposed framework. DenseNet-121
achieves the highest mean AUC-ROC (0.9754) across all four risk classes.
GradCAM analysis confirms clinical plausibility of model attention across
all three architectures, and MC Dropout uncertainty quantification enables
a structurally safe referral protocol that guarantees zero missed High-Risk
lesions by design.
